                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.chats.update", [$chat->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('message') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="message" id="message" value="{{ old('message', $chat->message) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('feature_type') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="feature_type" id="feature_type" value="{{ old('feature_type', $chat->feature_type) }}">
                        ...
                        <div class="form-group {{ $errors->has('feature') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\Chat::FEATURE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="feature_{{ $key }}" name="feature" value="{{ $key }}" {{ old('feature', $chat->feature) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('session') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="session_id" id="session_id" required>
                                @foreach($sessions as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('session_id') ? old('session_id') : $chat->session->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('message_from') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="message_from_id" id="message_from_id" required>
                                @foreach($message_froms as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('message_from_id') ? old('message_from_id') : $chat->message_from->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
